...
이전에 malloc을 분석하면서 gcc의 libstdc++ 코드를 살펴보게 되었는데 우연히 본 std::string 객체에 SSO가 있는걸 보고 여러 라이브러리에서 분석한 내용을 공유해볼까 해.
...
- SSO의 일반적인 구현
일단 기본적인 string 구조체를 의사 코드로 작성해볼게.
...
이 구조에서 길이 15이하의 문자열에 SSO를 적용시키면 이런 데이터 구조가 나올거야.
...
2) 최초 아이디어는 같지만 설계 철학에 따라 라이브러리마다 최종 구현부가 미묘하게 다른게 재밌다.
3) 잘 읽히지 않는게 정상.

지나치게 길어져서 2개로 나눌까 했는데 어차피 볼 사람만 보겠지 싶어서 그냥 둘게.
